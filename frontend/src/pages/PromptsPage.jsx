...
export default function PromptsPage() {
  ...
  return (
    <div className="space-y-3">
      <h1 className="text-4xl font-semibold tracking-tight" style={{ color: "var(--foreground)" }}>Prompts</h1>
      ...
      <Card>
        <div className="space-y-3">
          <div className="grid grid-cols-1 items-start gap-3 md:grid-cols-[minmax(220px,320px)_auto] md:justify-between">
            <div>
              <label className="mb-1 block text-[16px] font-medium" style={{ color: 'var(--foreground)' }}>Prompt name</label>
              <input
          ...
          <div>
            <label className="mb-1 block text-[16px] font-medium" style={{ color: 'var(--foreground)' }}>Template</label>
            <textarea
            ...
            />
            <p className="mt-1 text-[16px]" style={{ color: 'var(--muted-foreground)' }}>Tip: include {'{{input}}'} where dataset row input should be inserted.</p>
          </div>
        </div>
      </Card>
              ...
              <Card key={p.id}>
                <div className="flex items-start justify-between gap-3">
                  <button className="min-w-0 flex-1 text-left" onClick={() => toggle(p.id)}>
                    <p className="text-base font-semibold" style={{ color: 'var(--foreground)' }}>{p.name}</p>
                    <p className="mt-1 text-[16px]" style={{ color: 'var(--muted-foreground)' }}>{formatDateTime(p.created_at)}</p>
